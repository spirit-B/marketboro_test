...
router.post("/signup", async (req, res) => {
    ...
    try {
        const { userId, userName, password, confirmPW, category } = await userSchema.validateAsync(req.body);
        ...
        await User.create({ userId, userName, password: encrypted });
        ...
        if (joiError.includes("userId")) {
            ...
            return res.status(400).send({
                message: "비밀번호는 최소 8자 이상, 16자 이하의 영어 대소문자 및 숫자, 특수문자(!@#$%^*_-)입니다.",
            });
        }
    }
});
