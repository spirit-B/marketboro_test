...
router.post("/order", auth, async (req, res) => {
    ...
    try {
        for (let order of req.body) {
            const { productId, productName, orderQuantity } = order;
            await Order.create({ orderer: user.userId, productId, productName, orderQuantity });
        }
...
router.delete("/order/:orderId", auth, async (req, res) => {
    ...
    try {
        if (user.userId === order.orderer) {
            ...
            res.status(400).send({ message: "삭제 권한이 없습니다." });
        }
    } catch (error) {
